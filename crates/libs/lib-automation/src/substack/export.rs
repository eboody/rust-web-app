...
use lib_core::model::directus::{ArticleStatus, Articles, ArticlesSubstackStatus};
use ormlite::{query_builder::OnConflict, types::time::OffsetDateTime};
use substack::{config::config, prose_mirror::Node};
...
pub async fn export_draft(mm: &ModelManager, article_id: Uuid) -> Result<()> {
	...
	let response = mm
		...
		.headers(config().SUBSTACK_HEADERS.clone())
		.json(&payload)
		.send()
		.await?;
