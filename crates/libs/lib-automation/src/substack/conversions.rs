...
pub fn md_to_prosemirror(md: &str) -> Result<Document> {
	let prosemirror_output = Command::new("./scripts/to-prosemirror/mdtp.js")
		.arg(md)
		.output();
	dbg!("prosemirror_output: {}", &prosemirror_output);
	...
	for node in input.content.as_ref().unwrap() {
		...
		if is_citation_paragraph(node) {
			...
			if let Some(text_node) = citation_content.clone().first_mut() {
				text_node.text =
					text_node.clone().text.map(|t| t.trim().to_string());
			}
...
fn is_citation_paragraph(node: &Node) -> bool {
	...
	if_chain!(
		...
		if let Some(first_node) = &paragraph_content.first();
		if let Some(marks) = first_node.marks.as_ref();
		if let Some(first_mark) = marks.first();
		if let Some(attrs) = &first_mark.attrs;
		if let Some(href) = &attrs.href;
		if let Some(captures) = regex::Regex::new(r"#_?(end|foot|edn)(note|ref)?-?(\d+)")
			.unwrap()
			.captures(href);
		if let Some(number) = captures.get(3);
		then {
