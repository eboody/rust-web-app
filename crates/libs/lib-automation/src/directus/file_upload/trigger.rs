...
use lib_core::model::{directus::UploadFilePayload, ModelManager};
use serde::Serialize;
use uuid::Uuid;

use super::{article::on_article_upload, ebook::on_ebook_upload};
...
impl Content for UploadFilePayload {
	fn mime_type(&self) -> BoxResult<MimeType> {
		let mime_type = if self.type_ == "application/vnd.openxmlformats-officedocument.wordprocessingml.document" {
			MimeType::Docx
		} else if self.type_ == "application/pdf" {
			MimeType::Pdf
		} else {

			let err = Error::UnknownMimeType(
				self.clone()
			);
