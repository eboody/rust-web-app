...
#[derive(Debug)]
struct SectionDiff {
    to_create: Vec<model::Sections>,
    to_update: Vec<(model::Sections, lib_substack::Section)>,
    to_delete: Vec<lib_substack::Section>,
}

pub async fn sync_sections(mm: &ModelManager) -> Result<()> {
    // Fetch sections from both systems
    let directus_sections = model::Sections::select().fetch_all(mm.orm()).await?;
        ...
        .map(|(directus_section, substack_section)| async move {
                ...
                .await
            {
                Ok(updated) => {
                    ...
                    // Ensure the substack_id is synced back to Directus
                    directus_section
                        .update_partial()
                        .substack_id(substack_section.id)
                        .update(mm.orm())
                        .await?;
...
fn analyze_sections_diff(
    directus_sections: &[model::Sections],
...
fn needs_update(directus: &model::Sections, substack: &lib_substack::Section) -> bool {
    // Compare description strings directly
    Some(&directus.description) != Some(&substack.description)
        || directus.substack_id != substack.id
}
