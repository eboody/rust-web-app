...
use model::{Articles, Files, Issues};
use time::Month;
use uuid::Uuid;

pub async fn handle_audio(mm: &ModelManager, article: &Articles) -> Result<()> {
    let (volume, number) = calculate_volume_and_number(article.date_published.unwrap());
