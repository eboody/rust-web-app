...
pub fn config() -> &'static AutomationConfig {
  ...
  INSTANCE.get_or_init(|| {
    AutomationConfig::load_from_env()
...
pub struct AutomationConfig {
  ...
  pub ARTICLES_VIDEOS_FOLDER_ID: Uuid,
}
...
impl AutomationConfig {
  fn load_from_env() -> lib_utils::envs::Result<AutomationConfig> {
    ...
    anything_headers.insert(
      AUTHORIZATION,
      HeaderValue::from_str(&format!("Bearer {}", get_env("ANYTHINGLLM_KEY")?))
        .unwrap(),
    );
    ...
    Ok(AutomationConfig {
      ...
      .unwrap(),
      ARTICLES_VIDEOS_FOLDER_ID: Uuid::parse_str(&get_env(
        "ARTICLES_VIDEOS_FOLDER_ID",
      )?)
      .unwrap(),
    })
  }
