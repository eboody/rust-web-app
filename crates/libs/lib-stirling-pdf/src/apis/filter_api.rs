...
pub trait FilterApi: Send + Sync {
	async fn contains_image<'file_input, 'page_numbers>(
		&self,
		file_input: Option<std::path::PathBuf>,
		page_numbers: Option<&'page_numbers str>,
	) -> Result<Vec<String>, Error<ContainsImageError>>;
	async fn contains_text<'text, 'file_input, 'page_numbers>(
		&self,
		text: &'text str,
		file_input: Option<std::path::PathBuf>,
		page_numbers: Option<&'page_numbers str>,
	) -> Result<Vec<String>, Error<ContainsTextError>>;
	async fn file_size<'file_size, 'file_input, 'comparator>(
		&self,
		file_size: &'file_size str,
		file_input: Option<std::path::PathBuf>,
		comparator: Option<&'comparator str>,
	) -> Result<Vec<String>, Error<FileSizeError>>;
	async fn page_count<'file_input, 'comparator, 'page_count>(
		&self,
		file_input: Option<std::path::PathBuf>,
		comparator: Option<&'comparator str>,
		page_count: Option<&'page_count str>,
	) -> Result<Vec<String>, Error<PageCountError>>;
	async fn page_rotation<'rotation, 'file_input, 'comparator>(
		&self,
		rotation: i32,
		file_input: Option<std::path::PathBuf>,
		comparator: Option<&'comparator str>,
	) -> Result<Vec<String>, Error<PageRotationError>>;
	async fn page_size<'standard_page_size, 'file_input, 'comparator>(
		&self,
		standard_page_size: &'standard_page_size str,
		file_input: Option<std::path::PathBuf>,
		comparator: Option<&'comparator str>,
	) -> Result<Vec<String>, Error<PageSizeError>>;
}
...
impl FilterApi for FilterApiClient {
	...
	async fn file_size<'file_size, 'file_input, 'comparator>(
		...
		if let Some(local_var_param_value) = comparator {
			local_var_form =
				local_var_form.text("comparator", local_var_param_value.to_string());
		}
	...
	async fn page_count<'file_input, 'comparator, 'page_count>(
		...
		if let Some(local_var_param_value) = page_count {
			local_var_form =
				local_var_form.text("pageCount", local_var_param_value.to_string());
		}
		local_var_req_builder = local_var_req_builder.multipart(local_var_form);
	...
	async fn page_size<'standard_page_size, 'file_input, 'comparator>(
		...
		local_var_form =
			local_var_form.text("standardPageSize", standard_page_size.to_string());
		local_var_req_builder = local_var_req_builder.multipart(local_var_form);
