...
pub trait AssetsApi: Send + Sync {
    async fn get_asset<'id, 'key, 'transforms, 'download>(&self, id: &'id str, key: Option<&'key str>, transforms: Option<&'transforms str>, download: Option<bool>) -> Result<String, Error<GetAssetError>>;
}
...
impl AssetsApi for AssetsApiClient {
    ...
    async fn get_asset<'id, 'key, 'transforms, 'download>(&self, id: &'id str, key: Option<&'key str>, transforms: Option<&'transforms str>, download: Option<bool>) -> Result<String, Error<GetAssetError>> {
        ...
        if let Some(ref local_var_str) = transforms {
            local_var_req_builder = local_var_req_builder.query(&[("transforms", &local_var_str.to_string())]);
        }
