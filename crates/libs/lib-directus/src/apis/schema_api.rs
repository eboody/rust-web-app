...
pub trait SchemaApi: Send + Sync {
	async fn schema_apply<'schema_apply_request>(
		&self,
		schema_apply_request: models::SchemaApplyRequest,
	) -> Result<(), Error<SchemaApplyError>>;
	async fn schema_diff<'schema_snapshot200_response, 'force>(
		&self,
		schema_snapshot200_response: models::SchemaSnapshot200Response,
		force: Option<bool>,
	) -> Result<models::SchemaApplyRequest, Error<SchemaDiffError>>;
	async fn schema_snapshot<'export>(
		&self,
		export: Option<&'export str>,
	) -> Result<models::SchemaSnapshot200Response, Error<SchemaSnapshotError>>;
}
...
impl SchemaApi for SchemaApiClient {
	...
	async fn schema_apply<'schema_apply_request>(
		...
		let local_var_uri_str =
			format!("{}/schema/apply", local_var_configuration.base_path);
	...
	async fn schema_diff<'schema_snapshot200_response, 'force>(
		...
		let local_var_uri_str =
			format!("{}/schema/diff", local_var_configuration.base_path);
		let mut local_var_req_builder = local_var_client
			.request(reqwest::Method::POST, local_var_uri_str.as_str());
		...
		local_var_req_builder =
			local_var_req_builder.json(&schema_snapshot200_response);
	...
	async fn schema_snapshot<'export>(
		...
		let local_var_uri_str =
			format!("{}/schema/snapshot", local_var_configuration.base_path);
		let mut local_var_req_builder = local_var_client
			.request(reqwest::Method::GET, local_var_uri_str.as_str());

		if let Some(ref local_var_str) = export {
			local_var_req_builder = local_var_req_builder
				.query(&[("export", &local_var_str.to_string())]);
		}
		if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
			local_var_req_builder = local_var_req_builder
				.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
