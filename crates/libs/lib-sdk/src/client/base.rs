...
/// to the API, including GET, POST, PUT, PATCH, and DELETE operations.
...
impl Client {
	...
	where
		...
		P: Params,
	{
		let base_url = Url::parse(self.base_url.to_string().as_str())?;
		...
		let mut request_builder = match method {
			Method::GET => {
				...
				self.client
					.get(url)
					.headers(Self::default_headers(self.api_key.as_str())?)
			}
			Method::DELETE => self
				.client
				.delete(url)
	...
	fn map_error<T: Debug>(
		...
		match status {
            ...
            Some(StatusCode::CONFLICT) => Error::Conflict(format!("Conflict at '{}': API probably thinks this is a malformed resource. Error: {}. Body: {:?}", endpoint, message, body)),
            ...
            Some(StatusCode::INTERNAL_SERVER_ERROR) => Error::InternalServer(format!("Internal Server Error at '{}': Something went wrong on the server side. Error: {}. Body: {:?}", endpoint, message, body)),
		...
		T: DeserializeOwned + Debug,
	{
		...
		let mut next_url = Some(format!("{}{}", self.base_url, endpoint));
