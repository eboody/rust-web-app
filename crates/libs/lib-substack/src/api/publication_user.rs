...
impl PublicationUser {
    ...
    pub async fn get_by_user_id(client: &reqwest::Client, user_id: i64) -> Result<Option<Self>> {
    ...
    pub async fn get_by_name(client: &reqwest::Client, name: &str) -> Result<Option<Self>> {
        let users = Self::list(client).await?;
        let user = users.into_iter().find(|u| u.user.name == name);
        Ok(user)
    }

    pub fn to_byline(&self) -> ByLine {
        ByLine {
            id: self.user.id,
            is_guest: false,
        }
...
impl PublicationUserQuery {
    pub async fn execute(self, client: &reqwest::Client) -> Result<Vec<PublicationUser>> {
