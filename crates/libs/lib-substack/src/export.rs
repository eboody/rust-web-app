...
use crate::{config::config, prose_mirror::Node};
...
pub async fn export_draft(mm: &ModelManager, article_id: Uuid) -> Result<()> {
  ...
  let payload = draft::Request {
    audience: draft::Audience::Everyone,
    draft_body: draft::Body(json::to_string(&doc).unwrap()),
    draft_bylines: vec![draft::ByLine {
      id: 292604153,
      is_guest: false,
    }],
  ...
  let draft_response = response.json::<draft::Response>().await?;
