...
pub fn convert_docx_to_md(input_path: &Path, output_path: &Path) -> Result<()> {
    ...
    let markdown = htmd::convert(&html_string).expect("Failed to convert HTML to Markdown");
...
pub fn transform_to_substack_format(node: &mut Node) {
    if let NodeType::Text = node.type_ {
        if let Some(marks) = &node.marks {
            if let Some(link_mark) = marks.iter().find(|mark| mark.mark_type == MarkType::Link) {
...
fn is_citation_paragraph(node: &Node) -> bool {
    let is_paragraph_with_content = node.type_ == NodeType::Paragraph && node.content.is_some();
