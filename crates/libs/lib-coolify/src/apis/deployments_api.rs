...
pub trait DeploymentsApi: Send + Sync {
    async fn deploy_by_tag_or_uuid<'tag, 'uuid, 'force>(&self, tag: Option<&'tag str>, uuid: Option<&'uuid str>, force: Option<bool>) -> Result<models::DeployByTagOrUuid200Response, Error<DeployByTagOrUuidError>>;
    async fn get_deployment_by_uuid<'uuid>(&self, uuid: &'uuid str) -> Result<models::ApplicationDeploymentQueue, Error<GetDeploymentByUuidError>>;
    async fn list_deployments<>(&self, ) -> Result<Vec<models::ApplicationDeploymentQueue>, Error<ListDeploymentsError>>;
}
...
impl DeploymentsApi for DeploymentsApiClient {
    ...
    async fn deploy_by_tag_or_uuid<'tag, 'uuid, 'force>(&self, tag: Option<&'tag str>, uuid: Option<&'uuid str>, force: Option<bool>) -> Result<models::DeployByTagOrUuid200Response, Error<DeployByTagOrUuidError>> {
        ...
        let local_var_uri_str = format!("{}/deploy", local_var_configuration.base_path);
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<DeployByTagOrUuidError> = serde_json::from_str(&local_var_content).ok();
    ...
    async fn get_deployment_by_uuid<'uuid>(&self, uuid: &'uuid str) -> Result<models::ApplicationDeploymentQueue, Error<GetDeploymentByUuidError>> {
        ...
        let local_var_uri_str = format!("{}/deployments/{uuid}", local_var_configuration.base_path, uuid=crate::apis::urlencode(uuid));
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetDeploymentByUuidError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
