...
impl EmbedApi for EmbedApiClient {
	...
	async fn v1_embed_embed_uuid_chats_get<'embed_uuid>(
		...
		let local_var_uri_str = format!(
			...
			embedUuid = crate::apis::urlencode(embed_uuid)
		);
	...
	async fn v1_embed_embed_uuid_chats_session_uuid_get<
		'embed_uuid,
		'session_uuid,
	>(
		&self,
		...
		let local_var_uri_str = format!(
			...
			local_var_configuration.base_path,
			embedUuid = crate::apis::urlencode(embed_uuid),
			sessionUuid = crate::apis::urlencode(session_uuid)
		);
		let mut local_var_req_builder = local_var_client
			.request(reqwest::Method::GET, local_var_uri_str.as_str());
	...
	async fn v1_embed_get(&self) -> Result<json::Value, Error<V1EmbedGetError>> {
		...
		let local_var_uri_str =
			format!("{}/v1/embed", local_var_configuration.base_path);
		let mut local_var_req_builder = local_var_client
			.request(reqwest::Method::GET, local_var_uri_str.as_str());
